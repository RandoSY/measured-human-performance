...
def label_char(label):
    return next((c.upper() for c in label if c.isalnum()), "?")

async def display(client, mode, label=None):
    cmd = f"D,{mode},{label_char(label)}\n" if label else f"D,{mode}\n"
    try:
        await client.write_gatt_char(NUS_RX_UUID, cmd.encode(), response=True)
    except Exception:
        pass
...
async def main():
    ...
    with path.open("a",newline="",encoding="utf-8") as f:
        ...
        async with BleakClient(dev) as client:
            ...
            await client.write_gatt_char(NUS_RX_UUID,b"S,1\n",response=True)
            current="exercise"; await display(client,"CHAR",current)
            ...
            while True:
                ...
                if low.startswith("add "):
                    current=cmd[4:].strip() or current; await display(client,"CHAR",current); print(f"Current action: {current}"); continue
                if low=="list":
                ...
                if low=="one":
                    sid=max_ids[current]+1; await display(client,"RECORD",current); n=await capture_one(current,sid); max_ids[current]=sid
                    await display(client,"OK" if n else "ERROR"); await asyncio.sleep(.25); await display(client,"CHAR",current)
                    print(f"Saved sample {sid}: {n} readings"); continue
                if low=="ten":
                    for _ in range(10):
                        sid=max_ids[current]+1; await display(client,"RECORD",current); n=await capture_one(current,sid); max_ids[current]=sid
                        await display(client,"OK" if n else "ERROR"); print(f"Saved sample {sid}: {n} readings"); await asyncio.sleep(.25); await display(client,"CHAR",current)
                    continue
                if low=="cont10":
                    await display(client,"RECORD",current); n,counts=await capture_ten_continuous(current)
                    await display(client,"OK" if n else "ERROR"); await asyncio.sleep(.25); await display(client,"CHAR",current)
                    print(f"Saved {n} readings; per-sample counts={counts}"); continue
                print("Commands: add NAME | one | ten | cont10 | list | info | quit")
            await display(client,"STOP"); await client.write_gatt_char(NUS_RX_UUID,b"S,0\n",response=True); await client.stop_notify(NUS_TX_UUID)
    print(f"Saved: {path}")
